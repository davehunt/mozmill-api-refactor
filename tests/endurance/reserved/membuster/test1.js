...
const PREF_TAB_MODAL = "prompts.tab_modal.enabled";
...
function setupModule(aModule) {
  ...
  prefs.setPref(PREF_TAB_MODAL, false);
  
  // CHECK MODAL DIALOG HANDLING
}
...
function testMemBuster() {
  enduranceManager.run(function () {
    enduranceManager.loop(function () {
      ...
      if (currentEntity > 1) {
        if (tabBrowser.length < NUM_TABS) { //tabBrowser.length
          tabBrowser.openTab(); //tabBrowser.openTab()
        }
        else {
          var tabNum = currentEntity % NUM_TABS;
          controller.tabs.selectTabIndex(tabNum - 1); //controller.tabs.selectTabIndex(i)
        }
      }
      ...
      browser.openURL(LOCAL_TEST_PAGE, 0);
