...
/**
 * Returns true if Element is pointing at a valid node. This takes elasticity
 * into account, so by default it'll be whether the node exists within 5 sec.
 *
 * @name exists
 * @type boolean
 * @fieldOf widgets.Element#
...
/**
 * Returns true if Element is pointing at a valid node right now, false otherwise
 *
 * This will return the instantaneous status. That is, if the node
 * was once locatable but no longer is locatable, this will return false.
 *
 * @name existsNow
 * @type boolean
 * @fieldOf widgets.Element#
 */
Element.prototype.__defineGetter__("existsNow", function Element_getExistsNow() {
  return (this._locateElem() !== undefined);
});
