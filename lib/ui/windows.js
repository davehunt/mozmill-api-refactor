...
function ModalDialogObserver_findWindow() {
  ...
  let window = mozmill.utils.unwrapNode(driver.getLastOpenedWindow());

  // Get the WebBrowserChrome and check if it's a modal window
  let chrome = window.QueryInterface(Ci.nsIInterfaceRequestor).
  ...
  if (window.opener) {
    ...
    var opener = mozmill.utils.unwrapNode(window.opener);
...
function WindowWrapper(aDOMWindow) {
  ...
  let self = this;
  this._window.addEventListener("unload", function(event) {
    self.destroy();
  }, false);
...
function WindowWrapper__setDefaultModalDialogHandler() {
  this.initModalDialog(function (aWindow) {
